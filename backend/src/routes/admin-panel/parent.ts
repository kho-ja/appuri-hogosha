  ...
  uploadParentsFromKintone = async (req: ExtendedRequest, res: Response) => {
    ...
    try {
      if (
        ...
        throw new Error(
          'kintoneUrl, kintoneToken, given_name_field, family_name_field, email_field, phone_number_field, student_number_field are required'
        );
      }
      ...
      const existingPhoneNumbersSet = new Set(
        existingPhoneNumbers.map((phoneNumber: any) => phoneNumber.phone_number)
      );
      ...
      for (const row of updateList) {
        ...
        if (studentParent.length > 0) {
          errors.push({
            row,
            errors: { student_number: 'student_already_attached_to_parent' },
          });
        }
        await DB.execute(
      ...
      if (action === 'create') {
        for (const row of validResults) {
          if (existingEmails.includes(row.email)) {
            ...
            errors.push({
              row,
              errors: { phone_number: 'parent_phone_number_already_exists' },
            });
          } else {
            const parent = await this.cognitoClient.register(
              row.phone_number,
            ...
            if (
              ...
              if (studentRows.length > 0) {
                ...
                errors.push({
                  row,
                  errors: { student_numbers: 'invalid_student_numbers' },
                });
              }
            }
