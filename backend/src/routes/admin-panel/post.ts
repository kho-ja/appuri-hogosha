  ...
  uploadPostsFromCSV = async (req: ExtendedRequest, res: Response) => {
    ...
    try {
      ...
      for (const row of results) {
        const { title, description, priority, group_names, student_numbers } =
          row;
        const rowErrors: any = {};
      ...
      for (const post of validResults) {
        ...
        inserted.push({
          ...post,
          student_numbers: attachedStudents.map((e) => e.student_number),
      ...
      if (errors.length > 0) {
        ...
        if (withCSVBool) {
          const csvData = errors.map((error) => ({
            ...
            student_numbers: Array.isArray(error?.row?.student_numbers)
