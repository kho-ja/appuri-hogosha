...
class SchedulePostController implements IController {
  ...
  updateScheduledPost = async (req: ExtendedRequest, res: Response) => {
    try {
      ...
      const localTime = DateTime.fromISO(scheduled_at_string, {
        zone: 'utc',
  ...
  updateScheduledPostRecievers = async (
    ...
    try {
      ...
      if (values.length === 0) {
        return res.status(200).json({
          message: 'Receivers cleared successfully, nothing new to insert.',
        });
      }
      const updatedValues = values
        .map((value: any) => `(${value[0]}, ${value[1]}, ${value[2]})`)
  ...
  scheduledPostRecievers = async (req: ExtendedRequest, res: Response) => {
    try {
      ...
      const groupIds = recievers
        .filter((r: any) => r.group_id)
        .map((r: any) => r.group_id);
      const studentIds = recievers
