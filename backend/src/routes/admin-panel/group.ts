  ...
  uploadGroupsFromCSV = async (req: ExtendedRequest, res: Response) => {
    ...
    try {
      ...
      function handleInvalidGroup(
        ...
        const existingGroup = validResults.find((group) => group.name === name);
      ...
      function addErrorGroup(
        ...
        let existingError = errors.find((err) => err.row.name === row.name);
        if (existingError) {
      ...
      if (students && Array.isArray(students) && isValidArrayId(students)) {
        ...
        if (insertStudentIds.length > 0) {
          ...
          const valuesString = insertData
            .map((item: any) => `(${item.student_id}, ${item.group_id})`)
            .join(', ');
