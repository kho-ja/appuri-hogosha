...
async function checkUserStatus(phoneNumber: string): Promise<void> {
    ...
    try {
        ...
        const phoneVerifiedAttr = userResult.UserAttributes?.find(attr => attr.Name === 'phone_number_verified');
...
switch (command) {
    ...
    case 'verify-single':
        if (!phoneNumber) {
            console.log('❌ Please provide a phone number: npm run fix-phone verify-single +998935108199');
            process.exit(1);
        }
    ...
    case 'check-status':
        if (!phoneNumber) {
            console.log('❌ Please provide a phone number: npm run fix-phone check-status +998935108199');
            process.exit(1);
        }
