...
export const syncronizePosts = async (parentId: number, studentId: number) => {
    const parentStudents = await DB.query(`SELECT student_id
                                           FROM StudentParent
    ...
    const postStudents = await DB.query(`SELECT id
                                         FROM PostStudent
    ...
    const existingPostStudentIds = await DB.query(`SELECT post_student_id
                                                   FROM PostParent
                                                   WHERE parent_id = :parent_id
    ...
    const newPostStudentIds = postStudentIds.filter((id: any) => !existingPostStudentIds.find((ps: any) => ps.post_student_id === id));
    ...
    if (newPostStudentIds.length) {
        const insertValues = newPostStudentIds.map((id: any) => `(${id}, ${parentId}, 1)`).join(", ");
