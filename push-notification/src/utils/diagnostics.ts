...
export class DiagnosticsService {
    private pendingMessages = new Map<
        string,
        { phone: string; timestamp: number; attempts: number }
    >();
    ...
    updateStatus(
        ...
        if (message) {
            ...
            if (
                ['Delivered', 'Failed', 'Rejected', 'Expired'].includes(status)
            ) {
    ...
    async checkPendingMessages(): Promise<void> {
        ...
        for (const [messageId, message] of this.pendingMessages) {
            if (now - message.timestamp > staleThreshold) {
                console.warn(
                    `⚠️ SMS ${messageId} to ${message.phone} is stale (${Math.round((now - message.timestamp) / 60000)} minutes)`
                );
                console.warn(
                    `💡 Possible reasons: recipient phone off, out of coverage, or network issues`
                );
            }
        }
    }
    ...
    getDiagnostics(): DiagnosticsResult {
        ...
        const oldestTimestamp = Math.min(
            ...Array.from(this.pendingMessages.values()).map(m => m.timestamp)
        );
        const oldestAge =
            pending > 0
                ? Math.round((Date.now() - oldestTimestamp) / 60000)
                : 0;
    ...
    diagnoseSmsDeliveryIssue(phone: string, status?: string): void {
        ...
        if (!routing.isUzbekistan) {
            console.log(
                `❌ Invalid phone format: ${phone} (should be 998xxxxxxxxx)`
            );
            return;
        }
        ...
        if (status) {
            const statusMeanings: Record<string, string> = {
                Transmitted:
                    '📤 Sent to operator, waiting for delivery (normal - can take up to 24h)',
                Delivered: '✅ Successfully delivered to recipient',
                NotDelivered:
                    '❌ Not delivered (recipient has no credit, phone blocked, etc.)',
            ...
            if (status === 'Transmitted') {
                console.log(
                    `💡 "Transmitted" is normal - message sent to ${routing.operator}, awaiting delivery confirmation`
                );
                console.log(
                    `⏱️  Wait up to 24 hours for final delivery status`
                );
            }
        }
        ...
        console.log(
            `   1. If "Transmitted": Wait 24h, recipient may be offline/out of coverage`
        );
        console.log(
            `   2. If "NotDelivered": Recipient may have insufficient balance or be blocked`
        );
