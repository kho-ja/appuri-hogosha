...
export const generateSmsText = (post: NotificationPost): string => {
    ...
    if (post.language === 'jp') {
        return '新しい投稿: ' + post.title + ' に ' + post.family_name + ' リンク: ' + link;
    } else if (post.language === 'ru') {
        return 'Новый пост: ' + post.title + ' для ' + post.family_name + ' ссылка: ' + link;
    } else {
        return 'Yangi post: ' + post.title + ' uchun ' + post.family_name + ' havola: ' + link;
    }
};