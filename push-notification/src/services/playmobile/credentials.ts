    ...
    async verifyCredentials(): Promise<CredentialTestResult> {
        try {
            if (!ENVIRONMENT.BROKER_URL || !ENVIRONMENT.BROKER_AUTH) {
                return {
                    valid: false,
                    reason: 'Missing BROKER_URL or BROKER_AUTH environment variables',
                };
            }
            ...
            console.log(
                `   🗝️  Auth: Basic ${Buffer.from(ENVIRONMENT.BROKER_AUTH).toString('base64').substring(0, 10)}...`
            );
            ...
            if (response.status === 401) {
                return {
                    valid: false,
                    reason: 'Invalid credentials - check BROKER_AUTH',
                };
                ...
                try {
                    ...
                    if (errorCode === '202') {
                        ...
                        return {
                            valid: true,
                            reason: 'Credentials valid (got expected 202 for test number)',
                        };
                    } else {
                        return {
                            valid: true,
                            reason: `Credentials valid (got error ${errorCode} for test number: ${errorDescription})`,
                        };
                    }
                } catch (e) {
                    ...
                    return {
                        valid: true,
                        reason: 'Credentials valid (got 400 for test number)',
                    };
                }
                ...
                return {
                    valid: false,
                    reason: `Unexpected response: ${response.status} - ${responseText}`,
                };
            }
        } catch (error) {
            return {
